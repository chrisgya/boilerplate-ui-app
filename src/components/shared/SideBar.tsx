    ...
    return (
        <aside className="relative hidden h-screen text-gray-600 bg-white border-r shadow-xl sm:block">
            <nav className={cx("text-base font-semibold", isOpen && " w-56")}>
                ...
                <NavLink exact to="/change-password" activeClassName="active-nav-link" className={cx("flex items-center py-2 hover:opacity-100 nav-item", isOpen ? "ml-3" : "justify-center px-2")}>
                    <span data-tip={isOpen ? undefined : "Change Password"}><HomeIcon /></span>  {isOpen && <span className="ml-2">Change Password</span>}
                </NavLink>
            </nav>
